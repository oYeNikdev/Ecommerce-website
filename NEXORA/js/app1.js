...
//    CART
//    The cart is stored as [{ id, qty }]. Name, price and image are always
//    looked up from PRODUCTS in data.js, so prices can never go stale.
...
function initProductDetailsPage() {
  ...
  if (colorsEl && product.colors) {
    colorsEl.innerHTML = product.colors
      .map((c) => `<span class="detail-swatch" style="--swatch-color:${c.hex}" title="${c.name}"></span>`)
      .join('');
  }
